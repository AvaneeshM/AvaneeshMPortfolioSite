...
function buildCorpus(resume: Resume): CorpusChunk[] {
  ...
  chunks.push({
    id: 'about',
    title: 'About',
    text: [resume.about.tagline, resume.about.bio, resume.about.goals].filter(Boolean).join('\n'),
  })

  chunks.push({
    id: 'skills',
    title: 'Skills',
    text: resume.skills.map((g) => `${g.category}: ${g.items.join(', ')}`).join('\n'),
  })
  ...
  resume.projects.forEach((p, idx) => {
    chunks.push({
      ...
      text: [p.description, `Tech: ${p.tech.join(', ')}`].filter(Boolean).join('\n'),
    })
  })
  ...
  resume.experience.forEach((j, idx) => {
    chunks.push({
      ...
      text: [`${j.company} — ${j.role} (${j.dates})`, j.location, ...j.highlights, `Tech: ${j.tech.join(', ')}`]
        .filter(Boolean)
        .join('\n'),
    })
  })
...
function tfidf(tfMap: Map<string, number>, idf: Map<string, number>): Map<string, number> {
  const out = new Map<string, number>()
  for (const [t, v] of tfMap) out.set(t, v * (idf.get(t) ?? 0))
  return out
}

function extractRelevantSentences(chunkText: string, queryTokens: string[], maxSentences = 2): string[] {
...
export function answerFromResume(question: string, resume: Resume): ChatAnswer {
  ...
  const sources = top
    .map((t) => {
      ...
      return {
        title: t.chunk.title,
        snippet: sents.length ? sents.join(' ') : t.chunk.text.split('\n').slice(0, 2).join(' '),
      }
    })
  ...
  if (sources.length === 0) {
    return {
      answer:
        "I couldn't find that in the resume I was given. Try asking about skills, projects, experience, impact metrics, or specific technologies.",
      sources: [],
      suggestedQuestions,
    }
  }

  const bulletLines = sources.map((s) => `- ${s.snippet} (${s.title})`)
  const answer = `Based on the resume:\n${bulletLines.join('\n')}`
